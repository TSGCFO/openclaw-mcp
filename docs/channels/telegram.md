...
For complex replies (for example media payloads), OpenClaw falls back to normal final delivery and then cleans up the preview message.Preview streaming is separate from block streaming. When block streaming is explicitly enabled for Telegram, OpenClaw skips the preview stream to avoid double-streaming.Telegram-only reasoning stream:
...
`agentId` is topic-only and does not inherit from group defaults.**Per-topic agent routing**: Each topic can route to a different agent by setting `agentId` in the topic config. This gives each topic its own isolated workspace, memory, and session. Example:
...
### [​](https://docs.openclaw.ai/channels/telegram\#video-messages)  Video messages
...
Telegram supports exec approvals in approver DMs and can optionally post prompts in the originating chat or topic. Approvers must be numeric Telegram user IDs.Config path:
...
## [​](https://docs.openclaw.ai/channels/telegram\#configuration-reference)  Configuration reference

Primary reference: [Configuration reference - Telegram](https://docs.openclaw.ai/gateway/config-channels#telegram).
...
## [​](https://docs.openclaw.ai/channels/telegram\#related)  Related

[**Pairing** \\
\\
Pair a Telegram user to the gateway.](https://docs.openclaw.ai/channels/pairing)

[**Groups** \\
\\
Group and topic allowlist behavior.](https://docs.openclaw.ai/channels/groups)
...
[**Multi-agent routing** \\
\\
Map groups and topics to agents.](https://docs.openclaw.ai/concepts/multi-agent)

[**Troubleshooting** \\
\\
Cross-channel diagnostics.](https://docs.openclaw.ai/channels/troubleshooting)

[Slack](https://docs.openclaw.ai/channels/slack) [WhatsApp](https://docs.openclaw.ai/channels/whatsapp)
