---
source_url: https://docs.openclaw.ai/es/start/setup
title: "Configuraci\u00f3n - OpenClaw"
---

[Saltar al contenido principal](https://docs.openclaw.ai/es/start/setup#content-area)

[OpenClaw home page![light logo](https://mintcdn.com/clawdhub/dpADRo8IUoiDztzJ/assets/pixel-lobster.svg?fit=max&auto=format&n=dpADRo8IUoiDztzJ&q=85&s=8fdf719fb6d3eaad7c65231385bf28e5)![dark logo](https://mintcdn.com/clawdhub/dpADRo8IUoiDztzJ/assets/pixel-lobster.svg?fit=max&auto=format&n=dpADRo8IUoiDztzJ&q=85&s=8fdf719fb6d3eaad7c65231385bf28e5)](https://docs.openclaw.ai/es)

![ES](https://d3gk2c5xim1je2.cloudfront.net/flags/ES.svg)

Español

Buscar...

Ctrl K

Buscar...

Navigation

Advanced setup

Configuración

[Get started](https://docs.openclaw.ai/es) [Install](https://docs.openclaw.ai/es/install) [Channels](https://docs.openclaw.ai/es/channels) [Agents](https://docs.openclaw.ai/es/concepts/architecture) [Tools & Plugins](https://docs.openclaw.ai/es/tools) [Models](https://docs.openclaw.ai/es/providers) [Platforms](https://docs.openclaw.ai/es/platforms) [Gateway & Ops](https://docs.openclaw.ai/es/gateway) [Reference](https://docs.openclaw.ai/es/cli) [Help](https://docs.openclaw.ai/es/help)

En esta página

- [TL;DR](https://docs.openclaw.ai/es/start/setup#tldr)
- [Requisitos previos (desde código fuente)](https://docs.openclaw.ai/es/start/setup#requisitos-previos-desde-c%C3%B3digo-fuente)
- [Estrategia de personalización (para que las actualizaciones no duelan)](https://docs.openclaw.ai/es/start/setup#estrategia-de-personalizaci%C3%B3n-para-que-las-actualizaciones-no-duelan)
- [Ejecutar el Gateway desde este repositorio](https://docs.openclaw.ai/es/start/setup#ejecutar-el-gateway-desde-este-repositorio)
- [Flujo estable (primero la app de macOS)](https://docs.openclaw.ai/es/start/setup#flujo-estable-primero-la-app-de-macos)
- [Flujo de vanguardia (Gateway en una terminal)](https://docs.openclaw.ai/es/start/setup#flujo-de-vanguardia-gateway-en-una-terminal)
- [0) (Opcional) Ejecutar también la app de macOS desde código fuente](https://docs.openclaw.ai/es/start/setup#0-opcional-ejecutar-tambi%C3%A9n-la-app-de-macos-desde-c%C3%B3digo-fuente)
- [1) Iniciar el Gateway dev](https://docs.openclaw.ai/es/start/setup#1-iniciar-el-gateway-dev)
- [2) Apuntar la app de macOS a tu Gateway en ejecución](https://docs.openclaw.ai/es/start/setup#2-apuntar-la-app-de-macos-a-tu-gateway-en-ejecuci%C3%B3n)
- [3) Verificar](https://docs.openclaw.ai/es/start/setup#3-verificar)
- [Problemas comunes](https://docs.openclaw.ai/es/start/setup#problemas-comunes)
- [Mapa de almacenamiento de credenciales](https://docs.openclaw.ai/es/start/setup#mapa-de-almacenamiento-de-credenciales)
- [Actualizar (sin destrozar tu configuración)](https://docs.openclaw.ai/es/start/setup#actualizar-sin-destrozar-tu-configuraci%C3%B3n)
- [Linux (servicio de usuario de systemd)](https://docs.openclaw.ai/es/start/setup#linux-servicio-de-usuario-de-systemd)
- [Documentos relacionados](https://docs.openclaw.ai/es/start/setup#documentos-relacionados)

> ## Documentation Index
>
> Fetch the complete documentation index at: [https://docs.openclaw.ai/llms.txt](https://docs.openclaw.ai/llms.txt)
>
> Use this file to discover all available pages before exploring further.

Si estás configurando por primera vez, empieza con [Primeros pasos](https://docs.openclaw.ai/es/start/getting-started).
Para detalles de incorporación, consulta [Incorporación (CLI)](https://docs.openclaw.ai/es/start/wizard).

## [​](https://docs.openclaw.ai/es/start/setup\#tldr)  TL;DR

Elige un flujo de configuración según la frecuencia con la que quieras recibir actualizaciones y si quieres ejecutar el Gateway tú mismo:

- **La personalización vive fuera del repositorio:** mantén tu configuración y espacio de trabajo en `~/.openclaw/openclaw.json` y `~/.openclaw/workspace/` para que las actualizaciones del repositorio no los afecten.
- **Flujo estable (recomendado para la mayoría):** instala la app de macOS y deja que ejecute el Gateway incluido.
- **Flujo de vanguardia (dev):** ejecuta el Gateway tú mismo mediante `pnpm gateway:watch` y luego deja que la app de macOS se conecte en modo Local.

## [​](https://docs.openclaw.ai/es/start/setup\#requisitos-previos-desde-c%C3%B3digo-fuente)  Requisitos previos (desde código fuente)

- Node 24 recomendado (Node 22 LTS, actualmente `22.14+`, aún compatible)
- Se requiere `pnpm` para checkouts desde código fuente. OpenClaw carga plugins incluidos desde los paquetes del workspace pnpm `extensions/*` en modo dev, por lo que `npm install` en la raíz no prepara todo el árbol de código fuente.
- Docker (opcional; solo para configuración/e2e en contenedores; consulta [Docker](https://docs.openclaw.ai/es/install/docker))

## [​](https://docs.openclaw.ai/es/start/setup\#estrategia-de-personalizaci%C3%B3n-para-que-las-actualizaciones-no-duelan)  Estrategia de personalización (para que las actualizaciones no duelan)

Si quieres algo “100% adaptado a mí” _y_ actualizaciones sencillas, mantén tu personalización en:

- **Configuración:**`~/.openclaw/openclaw.json` (similar a JSON/JSON5)
- **Workspace:**`~/.openclaw/workspace` (skills, prompts, memorias; conviértelo en un repositorio git privado)

Inicializa una vez:

```
openclaw setup
```

Desde dentro de este repositorio, usa la entrada local de la CLI:

```
openclaw setup
```

Si aún no tienes una instalación global, ejecútala mediante `pnpm openclaw setup`.

## [​](https://docs.openclaw.ai/es/start/setup\#ejecutar-el-gateway-desde-este-repositorio)  Ejecutar el Gateway desde este repositorio

Después de `pnpm build`, puedes ejecutar la CLI empaquetada directamente:

```
node openclaw.mjs gateway --port 18789 --verbose
```

## [​](https://docs.openclaw.ai/es/start/setup\#flujo-estable-primero-la-app-de-macos)  Flujo estable (primero la app de macOS)

1. Instala e inicia **OpenClaw.app** (barra de menú).
2. Completa la lista de incorporación/permisos (solicitudes de TCC).
3. Asegúrate de que el Gateway esté en **Local** y en ejecución (la app lo gestiona).
4. Vincula superficies (ejemplo: WhatsApp):

```
openclaw channels login
```

5. Comprobación básica:

```
openclaw health
```

Si la incorporación no está disponible en tu compilación:

- Ejecuta `openclaw setup`, luego `openclaw channels login` y después inicia el Gateway manualmente (`openclaw gateway`).

## [​](https://docs.openclaw.ai/es/start/setup\#flujo-de-vanguardia-gateway-en-una-terminal)  Flujo de vanguardia (Gateway en una terminal)

Objetivo: trabajar en el Gateway de TypeScript, obtener recarga en caliente y mantener conectada la interfaz de la app de macOS.

### [​](https://docs.openclaw.ai/es/start/setup\#0-opcional-ejecutar-tambi%C3%A9n-la-app-de-macos-desde-c%C3%B3digo-fuente)  0) (Opcional) Ejecutar también la app de macOS desde código fuente

Si también quieres la app de macOS en la versión de vanguardia:

```
./scripts/restart-mac.sh
```

### [​](https://docs.openclaw.ai/es/start/setup\#1-iniciar-el-gateway-dev)  1) Iniciar el Gateway dev

```
pnpm install
# Solo la primera ejecución (o después de restablecer la configuración/workspace local de OpenClaw)
pnpm openclaw setup
pnpm gateway:watch
```

`gateway:watch` inicia o reinicia el proceso de observación del Gateway en una sesión tmux con nombre y se adjunta automáticamente desde terminales interactivas. Los shells no interactivos permanecen desacoplados e imprimen `tmux attach -t openclaw-gateway-watch-main`; usa `OPENCLAW_GATEWAY_WATCH_ATTACH=0 pnpm gateway:watch` para mantener desacoplada una ejecución interactiva, o `pnpm gateway:watch:raw` para el modo de observación en primer plano. El observador recarga ante cambios relevantes en código fuente, configuración y metadatos de plugins incluidos.
`pnpm openclaw setup` es el paso único de inicialización de configuración/workspace local para un checkout nuevo.
`pnpm gateway:watch` no recompila `dist/control-ui`, así que vuelve a ejecutar `pnpm ui:build` después de cambios en `ui/` o usa `pnpm ui:dev` mientras desarrollas la Control UI.

### [​](https://docs.openclaw.ai/es/start/setup\#2-apuntar-la-app-de-macos-a-tu-gateway-en-ejecuci%C3%B3n)  2) Apuntar la app de macOS a tu Gateway en ejecución

En **OpenClaw.app**:

- Modo de conexión: **Local**
La app se conectará al gateway en ejecución en el puerto configurado.

### [​](https://docs.openclaw.ai/es/start/setup\#3-verificar)  3) Verificar

- El estado del Gateway dentro de la app debería mostrar **“Usando gateway existente …”**
- O mediante CLI:

```
openclaw health
```

### [​](https://docs.openclaw.ai/es/start/setup\#problemas-comunes)  Problemas comunes

- **Puerto incorrecto:** el WS del Gateway usa por defecto `ws://127.0.0.1:18789`; mantén la app y la CLI en el mismo puerto.
- **Dónde vive el estado:**
  - Estado de canal/proveedor: `~/.openclaw/credentials/`
  - Perfiles de autenticación de modelo: `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`
  - Sesiones: `~/.openclaw/agents/<agentId>/sessions/`
  - Logs: `/tmp/openclaw/`

## [​](https://docs.openclaw.ai/es/start/setup\#mapa-de-almacenamiento-de-credenciales)  Mapa de almacenamiento de credenciales

Usa esto al depurar autenticación o decidir qué respaldar:

- **WhatsApp**: `~/.openclaw/credentials/whatsapp/<accountId>/creds.json`
- **Token de bot de Telegram**: config/env o `channels.telegram.tokenFile` (solo archivo normal; se rechazan symlinks)
- **Token de bot de Discord**: config/env o SecretRef (proveedores env/file/exec)
- **Tokens de Slack**: config/env (`channels.slack.*`)
- **Listas de permitidos de emparejamiento**:

  - `~/.openclaw/credentials/<channel>-allowFrom.json` (cuenta predeterminada)
  - `~/.openclaw/credentials/<channel>-<accountId>-allowFrom.json` (cuentas no predeterminadas)
- **Perfiles de autenticación de modelo**: `~/.openclaw/agents/<agentId>/agent/auth-profiles.json`
- **Payload de secretos respaldado por archivo (opcional)**: `~/.openclaw/secrets.json`
- **Importación OAuth heredada**: `~/.openclaw/credentials/oauth.json`
Más detalle: [Seguridad](https://docs.openclaw.ai/es/gateway/security#credential-storage-map).

## [​](https://docs.openclaw.ai/es/start/setup\#actualizar-sin-destrozar-tu-configuraci%C3%B3n)  Actualizar (sin destrozar tu configuración)

- Mantén `~/.openclaw/workspace` y `~/.openclaw/` como “tus cosas”; no pongas prompts/config personales en el repositorio `openclaw`.
- Actualizar código fuente: `git pull` \+ `pnpm install` \+ sigue usando `pnpm gateway:watch`.

## [​](https://docs.openclaw.ai/es/start/setup\#linux-servicio-de-usuario-de-systemd)  Linux (servicio de usuario de systemd)

Las instalaciones de Linux usan un servicio **de usuario** de systemd. De forma predeterminada, systemd detiene los servicios de usuario al cerrar sesión o quedar inactivo, lo que mata el Gateway. La incorporación intenta habilitar lingering por ti (puede pedir sudo). Si sigue desactivado, ejecuta:

```
sudo loginctl enable-linger $USER
```

Para servidores siempre activos o multiusuario, considera un servicio **del sistema** en lugar de un servicio de usuario (no requiere lingering). Consulta [Runbook del Gateway](https://docs.openclaw.ai/es/gateway) para las notas de systemd.

## [​](https://docs.openclaw.ai/es/start/setup\#documentos-relacionados)  Documentos relacionados

- [Runbook del Gateway](https://docs.openclaw.ai/es/gateway) (flags, supervisión, puertos)
- [Configuración del Gateway](https://docs.openclaw.ai/es/gateway/configuration) (esquema de configuración + ejemplos)
- [Discord](https://docs.openclaw.ai/es/channels/discord) y [Telegram](https://docs.openclaw.ai/es/channels/telegram) (etiquetas de respuesta + ajustes de replyToMode)
- [Configuración del asistente OpenClaw](https://docs.openclaw.ai/es/start/openclaw)
- [App de macOS](https://docs.openclaw.ai/es/platforms/macos) (ciclo de vida del gateway)

[Render](https://docs.openclaw.ai/es/install/render) [Flujo de trabajo de desarrollo de Pi](https://docs.openclaw.ai/es/pi-dev)

Ctrl+I