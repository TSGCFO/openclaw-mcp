...
title: "Plugin architecture internals - OpenClaw"
---
...
- [Plugin cache boundary](https://docs.openclaw.ai/plugins/architecture-internals#plugin-cache-boundary)
...
plugin.The activation planner exposes both an ids-only API for existing callers and a
...
### [​](https://docs.openclaw.ai/plugins/architecture-internals\#plugin-cache-boundary)  Plugin cache boundary
...
policy answers.The safe metadata fast path is explicit object ownership, not a hidden cache.
...
metadata caches.For plugin loading, the persistent cache layer is runtime loading. It may reuse
...
caller deliberately asked for runtime loading.Do not add persistent or wall-clock caches for:
...
that compatibility cleanup.If the provider needs a fully custom wire protocol or custom request executor,
that is a different class of extension. These hooks are for provider behavior
that still runs on OpenClaw’s normal inference loop.

### [​](https://docs.openclaw.ai/plugins/architecture-internals\#provider-example)  Provider example
...
### [​](https://docs.openclaw.ai/plugins/architecture-internals\#built-in-examples)  Built-in examples
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#runtime-helpers)  Runtime helpers
...
### [​](https://docs.openclaw.ai/plugins/architecture-internals\#api-runtime-imagegeneration)  `api.runtime.imageGeneration`
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#gateway-http-routes)  Gateway HTTP routes
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#plugin-sdk-import-paths)  Plugin SDK import paths
...
plugin fields. See [Channel plugins](https://docs.openclaw.ai/plugins/sdk-channel-plugins).Runtime and config helpers live under matching focused `*-runtime` subpaths
...
exists, then fall back to the resolved config file on disk.Capability-specific subpaths such as `image-generation`, `media-understanding`,
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#message-tool-schemas)  Message tool schemas
...
See [Message Presentation](https://docs.openclaw.ai/plugins/message-presentation) for the contract,
fallback rules, provider mapping, and plugin author checklist.Send-capable plugins declare what they can render through message capabilities:
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#channel-target-resolution)  Channel target resolution
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#config-backed-directories)  Config-backed directories

Plugins that derive directory entries from config should keep that logic in the
plugin and reuse the shared helpers from
`openclaw/plugin-sdk/directory-runtime`.Use this when a channel needs config-backed peers/groups such as:
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#provider-catalogs)  Provider catalogs

Provider plugins can define model catalogs for inference with
`registerProvider({ catalog: { run(...) { ... } } })`.`catalog.run(...)` returns the same shape OpenClaw writes into
`models.providers`:
...
defaults, or auth-gated model metadata.`catalog.order` controls when a plugin’s catalog merges relative to OpenClaw’s
...
built-in provider entry with the same provider id.Compatibility:

- `discovery` still works as a legacy alias
- if both `catalog` and `discovery` are registered, OpenClaw uses `catalog`

## [​](https://docs.openclaw.ai/plugins/architecture-internals\#read-only-channel-inspection)  Read-only channel inspection

If your plugin registers a channel, prefer implementing
`plugin.config.inspectAccount(cfg, accountId)` alongside `resolveAccount(...)`.Why:
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#package-packs)  Package packs
...
`node_modules` is available (`npm install` / `pnpm install`).Security guardrail: every `openclaw.extensions` entry must stay inside the plugin
directory after symlink resolution. Entries that escape the package directory are
rejected.Security note: `openclaw plugins install` installs plugin dependencies with a
...
`postinstall` builds.Optional: `openclaw.setupEntry` can point at a lightweight setup-only module.
...
code.Optional: `openclaw.startup.deferConfiguredChannelFullLoadUntilAfterListen`
can opt a channel plugin into the same `setupEntry` path during the gateway’s
pre-listen startup phase, even when the channel is already configured.Use this only when `setupEntry` fully covers the startup surface that must exist
...
full entry during startup.Bundled channels can also publish setup-only contract-surface helpers that core
...
`accounts.default`.Those setup patch adapters keep bundled contract-surface discovery lazy. Import
time stays light; the promotion surface is loaded only on first use instead of
re-entering bundled channel startup on module import.When those startup surfaces include gateway RPC methods, keep them on a
plugin-specific prefix. Core admin namespaces (`config.*`,
`exec.approvals.*`, `wizard.*`, `update.*`) remain reserved and always resolve
to `operator.admin`, even if a plugin requests a narrower scope.Example:
...
### [​](https://docs.openclaw.ai/plugins/architecture-internals\#channel-catalog-metadata)  Channel catalog metadata
...
contain `{ "entries": [ { "name": "@scope/pkg", "openclaw": { "channel": {...}, "install": {...} } } ] }`. The parser also accepts `"packages"` or `"plugins"` as legacy aliases for the `"entries"` key.Generated channel catalog entries and provider install catalog entries expose
...
importing plugin runtime.Official external npm entries should prefer an exact `npmSpec` plus
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#context-engine-plugins)  Context engine plugins
...
`plugins.slots.contextEngine`.Use this when your plugin needs to replace or extend the default context
...
values for construction-time initialization.If your engine does **not** own the compaction algorithm, keep `compact()`
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#adding-a-new-capability)  Adding a new capability

When a plugin needs behavior that does not fit the current API, do not bypass
the plugin system with a private reach-in. Add the missing capability.Recommended sequence:
...
provider’s worldview. See the [Capability Cookbook](https://docs.openclaw.ai/tools/capability-cookbook)
for a concrete file checklist and worked example.

### [​](https://docs.openclaw.ai/plugins/architecture-internals\#capability-checklist)  Capability checklist
...
### [​](https://docs.openclaw.ai/plugins/architecture-internals\#capability-template)  Capability template
...
## [​](https://docs.openclaw.ai/plugins/architecture-internals\#related)  Related

- [Plugin architecture](https://docs.openclaw.ai/plugins/architecture) — public capability model and shapes
...
[Internals](https://docs.openclaw.ai/plugins/architecture) [Skills](https://docs.openclaw.ai/tools/skills)
