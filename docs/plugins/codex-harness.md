...
- [Quick config](https://docs.openclaw.ai/plugins/codex-harness#quick-config)
...
- [Workspace bootstrap files](https://docs.openclaw.ai/plugins/codex-harness#workspace-bootstrap-files)
...
- [Common debugging workflow](https://docs.openclaw.ai/plugins/codex-harness#common-debugging-workflow)
- [Inspect a Codex thread from the CLI](https://docs.openclaw.ai/plugins/codex-harness#inspect-a-codex-thread-from-the-cli)
...
## [​](https://docs.openclaw.ai/plugins/codex-harness\#quick-config)  Quick config
...
`openai/gpt-5.5` without a Codex runtime override for direct API-key traffic.Legacy `codex/gpt-*` refs remain accepted as compatibility aliases. Doctor
...
OpenClaw on the protocol surface it has been tested against.For live and Docker smoke tests, auth usually comes from the Codex CLI account
...
## [​](https://docs.openclaw.ai/plugins/codex-harness\#workspace-bootstrap-files)  Workspace bootstrap files
...
`AGENTS.md` is missing.For OpenClaw workspace parity, the Codex harness resolves the other bootstrap
...
you intentionally want to run a different executable.By default, OpenClaw starts local Codex harness sessions in YOLO mode:
...
stopping on native approval prompts that nobody is around to answer.To opt in to Codex guardian-reviewed approvals, set `appServer.mode: "guardian"`:
...
but still need unattended agents to make progress.The `guardian` preset expands to `approvalPolicy: "on-request"`,
...
`auto_review`.For an already-running app-server, use WebSocket transport:
...
personal Codex CLI home.OpenClaw plugins and OpenClaw skill snapshots still flow through OpenClaw’s own
...
execute commands, expose MCP servers, or carry credentials.Auth is selected in this order:
...
remote app-server’s own account.If a deployment needs additional environment isolation, add those variables to
`appServer.clearEnv`:
...
`appServer.clearEnv` only affects the spawned Codex app-server child process.Codex dynamic tools default to the `native-first` profile. In that mode,
...
available.Supported top-level Codex plugin fields:
...
the turn can continue instead of leaving the session in `processing`.After OpenClaw responds to a Codex turn-scoped app-server request, the harness
...
native turn.Environment overrides remain available for local testing:
...
`appServer.command` is unset.`OPENCLAW_CODEX_APP_SERVER_GUARDIAN=1` was removed. Use
...
[Codex Computer Use](https://docs.openclaw.ai/plugins/codex-computer-use).The short version: OpenClaw does not vendor the desktop-control app or execute
...
MCP tool calls during Codex-mode turns.For direct TryCua driver access outside the Codex marketplace flow, register
`cua-driver mcp` with `openclaw mcp set cua-driver '{"command":"cua-driver","args":["mcp"]}'`.
See [Codex Computer Use](https://docs.openclaw.ai/plugins/codex-computer-use) for the distinction
between Codex-owned Computer Use and direct MCP registration.Minimal config:
...
[Codex Computer Use](https://docs.openclaw.ai/plugins/codex-computer-use) for marketplace choices,
remote catalog limits, status reasons, and troubleshooting.When `computerUse.autoInstall` is true, OpenClaw can register the standard
...
generic and works on any channel that supports OpenClaw text commands.Common forms:
...
### [​](https://docs.openclaw.ai/plugins/codex-harness\#common-debugging-workflow)  Common debugging workflow
...
thread ids together in one reply. See [Diagnostics export](https://docs.openclaw.ai/gateway/diagnostics)
for the full privacy model and group-chat behavior.Core OpenClaw also exposes owner-only `/diagnostics [note]` as the general
Gateway diagnostics command. Its approval prompt shows the sensitive-data
preamble, links to [Diagnostics Export](https://docs.openclaw.ai/gateway/diagnostics), and requests
...
it does not list Codex session or thread ids before approval.If `/diagnostics` is invoked by an owner in a group chat, OpenClaw keeps the
...
OpenClaw refuses the group request and asks the owner to run it from a DM.The approved Codex upload calls Codex app-server `feedback/upload` and asks
...
Gateway diagnostics export.`/codex resume` writes the same sidecar binding file that the harness uses for
...
### [​](https://docs.openclaw.ai/plugins/codex-harness\#inspect-a-codex-thread-from-the-cli)  Inspect a Codex thread from the CLI
...
`codex resume <thread-id>`. You can copy that command directly into a terminal.You can also get a thread id from `/codex binding` for the current chat or
`/codex threads [filter]` for recent Codex app-server threads, then run the same
`codex resume` command in your shell.The command surface requires Codex app-server `0.125.0` or newer. Individual
...
OpenClaw plugin hooks in the v1 contract.For OpenClaw dynamic tools, OpenClaw executes the tool after Codex asks for the
...
callbacks.Compaction and LLM lifecycle projections come from Codex app-server
...
of Codex’s internal request or compaction payloads.Codex native `hook/started` and `hook/completed` app-server notifications are
...
around that boundary.Supported in Codex runtime v1:
...
The Codex harness changes the low-level embedded agent executor only.OpenClaw still builds the tool list and receives dynamic tool results from the
harness. Text, images, video, music, TTS, approvals, and messaging-tool output
continue through the normal OpenClaw delivery path.The native hook relay is intentionally generic, but the v1 support contract is
...
it.For `PermissionRequest`, OpenClaw only returns explicit allow or deny decisions
when policy decides. A no-decision result is not an allow. Codex treats it as no
hook decision and falls through to its own guardian or user approval path.Codex MCP tool approval elicitations are routed through OpenClaw’s plugin
...
requests still fail closed.Active-run queue steering maps onto Codex app-server `turn/steer`. With the
...
[Steering queue](https://docs.openclaw.ai/concepts/queue-steering).When the selected model uses the Codex harness, native thread compaction is
...
kept after compaction.Because Codex owns the canonical native thread, `tool_result_persist` does not
currently rewrite Codex-native tool result records. It only applies when
OpenClaw is writing an OpenClaw-owned session transcript tool result.Media generation does not require PI. Image, video, music, PDF, TTS, and media
...
`codex`.**OpenClaw uses PI instead of Codex:**`agentRuntime.id: "auto"` can still use PI as the
...
is selected, its failures surface directly.**The app-server is rejected:** upgrade Codex so the app-server handshake
...
stable `0.125.0` protocol floor is what OpenClaw tests.**Model discovery is slow:** lower `plugins.entries.codex.config.discovery.timeoutMs`
or disable discovery.**WebSocket transport fails immediately:** check `appServer.url`, `authToken`,
and that the remote app-server speaks the same Codex app-server protocol version.**A non-Codex model uses PI:** that is expected unless you forced
...
turn for that agent must be a Codex-supported OpenAI model.**Computer Use is installed but tools do not run:** check
...
- [Testing](https://docs.openclaw.ai/help/testing-live#live-codex-app-server-harness-smoke)

[Dependencies](https://docs.openclaw.ai/plugins/dependency-resolution) [Codex Computer Use](https://docs.openclaw.ai/plugins/codex-computer-use)
