...
title: "Plugin hooks - OpenClaw"
---

[Skip to main content](https://docs.openclaw.ai/plugins/hooks#content-area)
...
- [Quick start](https://docs.openclaw.ai/plugins/hooks#quick-start)
- [Hook catalog](https://docs.openclaw.ai/plugins/hooks#hook-catalog)
- [Tool call policy](https://docs.openclaw.ai/plugins/hooks#tool-call-policy)
- [Tool result persistence](https://docs.openclaw.ai/plugins/hooks#tool-result-persistence)
- [Prompt and model hooks](https://docs.openclaw.ai/plugins/hooks#prompt-and-model-hooks)
- [Session extensions and next-turn injections](https://docs.openclaw.ai/plugins/hooks#session-extensions-and-next-turn-injections)
- [Message hooks](https://docs.openclaw.ai/plugins/hooks#message-hooks)
- [Install hooks](https://docs.openclaw.ai/plugins/hooks#install-hooks)
- [Gateway lifecycle](https://docs.openclaw.ai/plugins/hooks#gateway-lifecycle)
- [Upcoming deprecations](https://docs.openclaw.ai/plugins/hooks#upcoming-deprecations)
- [Related](https://docs.openclaw.ai/plugins/hooks#related)
...
## [​](https://docs.openclaw.ai/plugins/hooks\#quick-start)  Quick start
...
## [​](https://docs.openclaw.ai/plugins/hooks\#hook-catalog)  Hook catalog
...
## [​](https://docs.openclaw.ai/plugins/hooks\#tool-call-policy)  Tool call policy
...
### [​](https://docs.openclaw.ai/plugins/hooks\#tool-result-persistence)  Tool result persistence
...
## [​](https://docs.openclaw.ai/plugins/hooks\#prompt-and-model-hooks)  Prompt and model hooks
...
so your plugin does not depend on a legacy combined phase.`before_agent_start` and `agent_end` include `event.runId` when OpenClaw can
...
job.For channel-originated runs, `ctx.messageProvider` is the provider surface such
...
metadata.`agent_end` is an observation hook and runs fire-and-forget after the turn. The
...
plugin also uses its own abort signal.Use `model_call_started` and `model_call_ended` for provider-call telemetry
...
bounded provider request-id hash.`before_agent_finalize` runs only when a harness is about to accept a natural
...
`before_agent_finalize` decisions.Non-bundled plugins that need `llm_input`, `llm_output`,
...
### [​](https://docs.openclaw.ai/plugins/hooks\#session-extensions-and-next-turn-injections)  Session extensions and next-turn injections
...
plugin-owned status without learning plugin internals.Use `api.enqueueNextTurnInjection(...)` when a plugin needs durable context to
...
the model on the next turn but should not become permanent system prompt text.Cleanup semantics are part of the contract. Session extension cleanup and
...
## [​](https://docs.openclaw.ai/plugins/hooks\#message-hooks)  Message hooks
...
media caption.Message hook contexts expose stable correlation fields when available:
`ctx.sessionKey`, `ctx.runId`, `ctx.messageId`, `ctx.senderId`, `ctx.trace`,
`ctx.traceId`, `ctx.spanId`, `ctx.parentSpanId`, and `ctx.callDepth`. Prefer
these first-class fields before reading legacy metadata.Prefer typed `threadId` and `replyToId` fields before using channel-specific
...
## [​](https://docs.openclaw.ai/plugins/hooks\#install-hooks)  Install hooks
...
## [​](https://docs.openclaw.ai/plugins/hooks\#gateway-lifecycle)  Gateway lifecycle
...
resources.Do not rely on the internal `gateway:startup` hook for plugin-owned runtime
services.`cron_changed` fires for gateway-owned cron lifecycle events with a typed
...
## [​](https://docs.openclaw.ai/plugins/hooks\#upcoming-deprecations)  Upcoming deprecations
...
[Plaintext channel envelopes → BodyForAgent](https://docs.openclaw.ai/plugins/sdk-migration#active-deprecations).
...
[Plugin SDK migration → Active deprecations](https://docs.openclaw.ai/plugins/sdk-migration#active-deprecations).

## [​](https://docs.openclaw.ai/plugins/hooks\#related)  Related

- [Plugin SDK migration](https://docs.openclaw.ai/plugins/sdk-migration) — active deprecations and removal timeline
...
[Getting Started](https://docs.openclaw.ai/plugins/building-plugins) [Channel Plugins](https://docs.openclaw.ai/plugins/sdk-channel-plugins)
