...
- [Generation provider metadata reference](https://docs.openclaw.ai/plugins/manifest#generation-provider-metadata-reference)
- [Tool metadata reference](https://docs.openclaw.ai/plugins/manifest#tool-metadata-reference)
...
## [​](https://docs.openclaw.ai/plugins/manifest\#generation-provider-metadata-reference)  Generation provider metadata reference
...
provider plugin.Use these fields only for cheap, declarative facts. Transport, request
...
## [​](https://docs.openclaw.ai/plugins/manifest\#tool-metadata-reference)  Tool metadata reference
...
should not change correctness while manifest ownership fallbacks still exist.Every plugin should set `activation.onStartup` intentionally. Set it to `true`
...
## [​](https://docs.openclaw.ai/plugins/manifest\#qarunners-reference)  qaRunners reference
...
## [​](https://docs.openclaw.ai/plugins/manifest\#setup-reference)  setup reference
...
control-plane/setup flows that should stay metadata-only.When present, `setup.providers` and `setup.cliBackends` are the preferred
...
fallback execution path.OpenClaw also includes `setup.providers[].envVars` in generic provider auth and
...
on `setup.providers[].envVars`.OpenClaw can also derive simple setup choices from `setup.providers[].authMethods`
...
preferred for custom labels, CLI flags, onboarding scope, and assistant metadata.Set `requiresRuntime: false` only when those descriptors are sufficient for the
...
descriptors without the flag do not break.Because setup lookup can execute plugin-owned `setup-api` code, normalized
...
winner from discovery order.When setup runtime does execute, setup registry diagnostics report descriptor
...
### [​](https://docs.openclaw.ai/plugins/manifest\#setup-providers-reference)  setup.providers reference
...
provider API probes.Supported evidence entries:
...
### [​](https://docs.openclaw.ai/plugins/manifest\#setup-fields)  setup fields
...
## [​](https://docs.openclaw.ai/plugins/manifest\#uihints-reference)  uiHints reference
...
## [​](https://docs.openclaw.ai/plugins/manifest\#contracts-reference)  contracts reference
...
output before the model sees it.Runtime `api.registerTool(...)` registrations must match `contracts.tools`.
Tool discovery uses this list to load only the plugin runtimes that can own the
requested tools.Provider plugins that implement `resolveExternalAuthProfiles` should declare
...
will be removed after the migration window.Bundled memory embedding providers should declare
...
## [​](https://docs.openclaw.ai/plugins/manifest\#mediaunderstandingprovidermetadata-reference)  mediaUnderstandingProviderMetadata reference
...
## [​](https://docs.openclaw.ai/plugins/manifest\#channelconfigs-reference)  channelConfigs reference
...
when `setup.requiresRuntime: false` declares setup runtime unnecessary.`channelConfigs` is plugin manifest metadata, not a new top-level user config
...
channel before plugin runtime code executes.For a channel plugin, `configSchema` and `channelConfigs` describe different
paths:
...
channel-owned option shape until plugin runtime executes.`channelConfigs.<channel-id>.commands.nativeCommandsAutoEnabled` and
...
### [​](https://docs.openclaw.ai/plugins/manifest\#replacing-another-channel-plugin)  Replacing another channel plugin
...
silently changing the requested plugin set.Keep `preferOver` scoped to plugin ids that can really provide the same channel.
It is not a general priority field and it does not rename user config keys.

## [​](https://docs.openclaw.ai/plugins/manifest\#modelsupport-reference)  modelSupport reference
...
## [​](https://docs.openclaw.ai/plugins/manifest\#modelcatalog-reference)  modelCatalog reference
...
canonical provider rows only.`suppressions` replaces the old provider runtime `suppressBuiltInModel` hook.
...
suppression hooks are no longer called during model resolution.Provider fields:
...
## [​](https://docs.openclaw.ai/plugins/manifest\#modelidnormalization-reference)  modelIdNormalization reference
...
## [​](https://docs.openclaw.ai/plugins/manifest\#providerendpoints-reference)  providerEndpoints reference
...
`endpointClass`; plugin manifests own the host and base URL metadata.Endpoint fields:
...
## [​](https://docs.openclaw.ai/plugins/manifest\#providerrequest-reference)  providerRequest reference
...
## [​](https://docs.openclaw.ai/plugins/manifest\#modelpricing-reference)  modelPricing reference
...
### [​](https://docs.openclaw.ai/plugins/manifest\#openclaw-provider-index)  OpenClaw Provider Index
...
model picker surfaces will consume when a provider plugin is not installed.Catalog authority order:
...
making normal listing or onboarding call provider APIs.Provider Index entries may also carry installable-plugin metadata for providers
...
the Provider Index entry is ignored for that provider.Legacy top-level capability keys are deprecated. Use `openclaw doctor --fix` to
...
## [​](https://docs.openclaw.ai/plugins/manifest\#manifest-versus-package-json)  Manifest versus package.json
...
### [​](https://docs.openclaw.ai/plugins/manifest\#package-json-fields-that-affect-discovery)  package.json fields that affect discovery
...
`package.json#openclaw` fields below.Important examples:
...
choices. Do not move install hints into `openclaw.plugin.json`.`openclaw.install.minHostVersion` is enforced during install and manifest
...
plugins are assumed to be co-versioned with the host checkout.Official install-on-demand metadata should use `clawhubSpec` when the plugin is
...
fallback for packages that have not moved to ClawHub yet.Exact npm version pinning already lives in `npmSpec`, for example
...
recorded without an integrity pin.Channel plugins should provide `openclaw.setupEntry` when status, channel list,
...
transport runtimes in the main extension entrypoint.Runtime entrypoint fields do not override package-boundary checks for source
entrypoint fields. For example, `openclaw.runtimeExtensions` cannot make an
escaping `openclaw.extensions` path loadable.`openclaw.install.allowInvalidConfigRecovery` is intentionally narrow. It does
...
to `openclaw doctor --fix`.`openclaw.channel.persistedAuthState` is package metadata for a tiny checker
module:
...
not route it through the full channel runtime barrel.`openclaw.channel.configuredState` follows the same shape for cheap env-only
configured checks:
...
## [​](https://docs.openclaw.ai/plugins/manifest\#discovery-precedence-duplicate-plugin-ids)  Discovery precedence (duplicate plugin ids)

OpenClaw discovers plugins from several roots (bundled, global install, workspace, explicit config-selected paths). If two discoveries share the same `id`, only the **highest-precedence** manifest is kept; lower-precedence duplicates are dropped instead of loading beside it.Precedence, highest to lowest:
...
## [​](https://docs.openclaw.ai/plugins/manifest\#related)  Related

[**Building plugins** \\
\\
Getting started with plugins.](https://docs.openclaw.ai/plugins/building-plugins)

[**Plugin architecture** \\
\\
Internal architecture and capability model.](https://docs.openclaw.ai/plugins/architecture)

[**SDK overview** \\
\\
Plugin SDK reference and subpath imports.](https://docs.openclaw.ai/plugins/sdk-overview)
