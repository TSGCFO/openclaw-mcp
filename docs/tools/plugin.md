...
- [OpenClaw-owned npm packages during migration](https://docs.openclaw.ai/tools/plugin#openclaw-owned-npm-packages-during-migration)
...
- [Slow plugin tool setup](https://docs.openclaw.ai/tools/plugin#slow-plugin-tool-setup)
...
[ClawHub](https://docs.openclaw.ai/tools/clawhub). Npm remains supported for direct installs and for a
...
spec through npm.If config is invalid, install normally fails closed and points you at
...
running doctor cleanup if you want stale plugin ids removed.Plugin dependency installation happens only during explicit install/update or
...
See [Plugin dependency resolution](https://docs.openclaw.ai/plugins/dependency-resolution) for the
install-time lifecycle.For npm installs, mutable selectors such as `latest` or a dist-tag are resolved
...
is rolled back instead of accepting a different plugin artifact.Source checkouts are pnpm workspaces. If you clone OpenClaw to hack on bundled
...
### [​](https://docs.openclaw.ai/tools/plugin\#openclaw-owned-npm-packages-during-migration)  OpenClaw-owned npm packages during migration
...
npm for older/custom installs and direct npm workflows.If npm reports an `@openclaw/*` plugin package as deprecated, that package
...
| Plugin | Package | Docs |
| --- | --- | --- |
| BlueBubbles | `@openclaw/bluebubbles` | [BlueBubbles](https://docs.openclaw.ai/channels/bluebubbles) |
| Discord | `@openclaw/discord` | [Discord](https://docs.openclaw.ai/channels/discord) |
| Feishu | `@openclaw/feishu` | [Feishu](https://docs.openclaw.ai/channels/feishu) |
| Matrix | `@openclaw/matrix` | [Matrix](https://docs.openclaw.ai/channels/matrix) |
| Mattermost | `@openclaw/mattermost` | [Mattermost](https://docs.openclaw.ai/channels/mattermost) |
| Microsoft Teams | `@openclaw/msteams` | [Microsoft Teams](https://docs.openclaw.ai/channels/msteams) |
| Nextcloud Talk | `@openclaw/nextcloud-talk` | [Nextcloud Talk](https://docs.openclaw.ai/channels/nextcloud-talk) |
| Nostr | `@openclaw/nostr` | [Nostr](https://docs.openclaw.ai/channels/nostr) |
| Synology Chat | `@openclaw/synology-chat` | [Synology Chat](https://docs.openclaw.ai/channels/synology-chat) |
| Tlon | `@openclaw/tlon` | [Tlon](https://docs.openclaw.ai/channels/tlon) |
| WhatsApp | `@openclaw/whatsapp` | [WhatsApp](https://docs.openclaw.ai/channels/whatsapp) |
...
shape.Config changes made through `/plugins enable` or `/plugins disable` trigger an
...
plugin modules cannot be safely replaced in place.`openclaw plugins list` is a local plugin registry/config snapshot. An
...
### [​](https://docs.openclaw.ai/tools/plugin\#slow-plugin-tool-setup)  Slow plugin tool setup
...
least 1s or total plugin tool factory prep takes at least 5s.OpenClaw caches successful plugin tool factory results for repeated resolutions
...
depend on those trusted fields are re-run when the context changes.If one plugin dominates the timing, inspect its runtime registrations:
...
plugin). Other bundled plugins still need `openclaw plugins enable <id>`.`--force` overwrites an existing installed plugin or hook pack in place. Use
...
of copying over a managed install target.When `plugins.allow` is already set, `openclaw plugins install` adds the
...
explicit install is immediately loadable after restart.OpenClaw keeps a persisted local plugin registry as the cold read model for
...
beta release exists. Exact versions and explicit tags stay pinned.`--pin` is npm-only. It is not supported with `--marketplace`, because
marketplace installs persist marketplace source metadata instead of an npm spec.`--dangerously-force-unsafe-install` is a break-glass override for false
...
those names.This CLI flag applies to plugin install/update flows only. Gateway-backed skill
dependency installs use the matching `dangerouslyForceUnsafeInstall` request
override instead, while `openclaw skills install` remains the separate ClawHub
skill download/install flow.If a plugin you published on ClawHub is hidden or blocked by a scan, open the
...
public.Compatible bundles participate in the same plugin list/inspect/enable/disable
...
directories.`openclaw plugins inspect <id>` also reports detected bundle capabilities plus
supported or unsupported MCP and LSP server entries for bundle-backed plugins.Marketplace sources can be a Claude known-marketplace name from
...
cloned marketplace repo and use relative path sources only.See [`openclaw plugins` CLI reference](https://docs.openclaw.ai/cli/plugins) for full details.

## [​](https://docs.openclaw.ai/tools/plugin\#plugin-api-overview)  Plugin API overview
...
public contract.`api.registrationMode` tells a plugin why its entry is being loaded:
...
behind full-runtime paths.Common registration methods:
...
[Codex harness v1 support contract](https://docs.openclaw.ai/plugins/codex-harness#v1-support-contract).For full typed hook behavior, see [SDK overview](https://docs.openclaw.ai/plugins/sdk-overview#hook-decision-semantics).

## [​](https://docs.openclaw.ai/tools/plugin\#related)  Related
...
[Tools and plugins](https://docs.openclaw.ai/tools) [Manage plugins](https://docs.openclaw.ai/plugins/manage-plugins)
