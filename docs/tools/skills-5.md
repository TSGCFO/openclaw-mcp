---
source_url: https://docs.openclaw.ai/tools/skills.md
title: ""
---
...
Codex CLI's native \`$CODEX\_HOME/skills\` directory is not one of these OpenClaw
...
\| Action \| Command \|
\| \-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\- \| \-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\-\- \|
\| Install a skill into the workspace \| \`openclaw skills install \` \|
...
\\* \`openclaw skills install \` is different — it downloads a ClawHub skill folder into the workspace and does not use the installer-metadata path above.
...
metadata:
 ...
 "openclaw":
 {
 "requires": { "bins": \["uv"\], "env": \["GEMINI\_API\_KEY"\], "config": \["browser.enabled"\] },
 "primaryEnv": "GEMINI\_API\_KEY",
 },
 }
\-\-\-
\`\`\`
...
\`\`\`json5 theme={"theme":{"light":"min-light","dark":"min-dark"}}
{
 ...
 env: {
 GEMINI\_API\_KEY: "GEMINI\_KEY\_HERE",
 },
 config: {
...
defines \`metadata.openclaw.skillKey\`, use that key under \`skills.entries\`.
 ...
 \`image\_generate\` tool with \`agents.defaults.imageGenerationModel\` instead
...
total = 195 + Σ (97 + len(name\_escaped) + len(description\_escaped) + len(location\_escaped))
\`\`\`
