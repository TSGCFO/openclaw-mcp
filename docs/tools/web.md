...
[**Kimi** \\
\\
AI-synthesized answers with citations via Moonshot web search; ungrounded chat fallbacks fail explicitly.](https://docs.openclaw.ai/tools/kimi-search)

[**MiniMax Search** \\
\\
Structured results via the MiniMax Token Plan search API.](https://docs.openclaw.ai/tools/minimax-search)
...
| [Kimi](https://docs.openclaw.ai/tools/kimi-search) | AI-synthesized + citations; fails on ungrounded chat fallbacks | — | `KIMI_API_KEY` / `MOONSHOT_API_KEY` |
| [MiniMax Search](https://docs.openclaw.ai/tools/minimax-search) | Structured snippets | Region (`global` / `cn`) | `MINIMAX_CODE_PLAN_KEY` / `MINIMAX_CODING_API_KEY` / `MINIMAX_OAUTH_TOKEN` |
...
provider pages for examples.`tools.web.search.provider` is validated against the web-search provider ids
...
plugin or run `openclaw doctor --fix` to clean up the stale config.`web_fetch` fallback provider selection is separate:
...
[Web Fetch](https://docs.openclaw.ai/tools/web-fetch) [Brave search](https://docs.openclaw.ai/tools/brave-search)
