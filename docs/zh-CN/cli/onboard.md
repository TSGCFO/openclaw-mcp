...
[跳转到主要内容](https://docs.openclaw.ai/zh-CN/cli/onboard#content-area)
...
- [openclaw onboard](https://docs.openclaw.ai/zh-CN/cli/onboard#openclaw-onboard)
- [相关指南](https://docs.openclaw.ai/zh-CN/cli/onboard#%E7%9B%B8%E5%85%B3%E6%8C%87%E5%8D%97)
- [示例](https://docs.openclaw.ai/zh-CN/cli/onboard#%E7%A4%BA%E4%BE%8B)
- [非交互式 Z.AI 端点选择](https://docs.openclaw.ai/zh-CN/cli/onboard#%E9%9D%9E%E4%BA%A4%E4%BA%92%E5%BC%8F-z-ai-%E7%AB%AF%E7%82%B9%E9%80%89%E6%8B%A9)
- [流程说明](https://docs.openclaw.ai/zh-CN/cli/onboard#%E6%B5%81%E7%A8%8B%E8%AF%B4%E6%98%8E)
- [常用后续命令](https://docs.openclaw.ai/zh-CN/cli/onboard#%E5%B8%B8%E7%94%A8%E5%90%8E%E7%BB%AD%E5%91%BD%E4%BB%A4)
...
# [​](https://docs.openclaw.ai/zh-CN/cli/onboard\#openclaw-onboard)  `openclaw onboard`

用于本地或远程 Gateway 网关设置的交互式新手引导。

## [​](https://docs.openclaw.ai/zh-CN/cli/onboard\#%E7%9B%B8%E5%85%B3%E6%8C%87%E5%8D%97)  相关指南

[**CLI 新手引导中心** \\
\\
交互式 CLI 流程演练。](https://docs.openclaw.ai/zh-CN/start/wizard)

[**新手引导概览** \\
\\
OpenClaw 新手引导如何组合在一起。](https://docs.openclaw.ai/zh-CN/start/onboarding-overview)

[**CLI 设置参考** \\
\\
输出、内部机制和每步行为。](https://docs.openclaw.ai/zh-CN/start/wizard-cli-reference)

[**CLI 自动化** \\
\\
非交互式标志和脚本化设置。](https://docs.openclaw.ai/zh-CN/start/wizard-cli-automation)

[**macOS 应用新手引导** \\
\\
macOS 菜单栏应用的新手引导流程。](https://docs.openclaw.ai/zh-CN/start/onboarding)

## [​](https://docs.openclaw.ai/zh-CN/cli/onboard\#%E7%A4%BA%E4%BE%8B)  示例
...
`--flow import` 使用插件拥有的迁移提供商，例如 Hermes。它只会针对全新的 OpenClaw 设置运行；如果已有配置、凭据、会话，或工作区记忆/身份文件，请在导入前重置或选择全新设置。`--modern` 会启动 Crestodian 对话式新手引导预览版。不使用
`--modern` 时，`openclaw onboard` 会保留经典新手引导流程。对于明文私有网络 `ws://` 目标（仅限受信任网络），请在新手引导进程环境中设置
`OPENCLAW_ALLOW_INSECURE_PRIVATE_WS=1`。
这个客户端侧传输协议应急开关没有对应的 `openclaw.json` 配置项。非交互式自定义提供商：
...
OpenClaw 会自动将常见视觉模型 ID 标记为支持图像。对于未知的自定义视觉 ID，请传入 `--custom-image-input`；或传入 `--custom-text-input` 强制使用仅文本元数据。LM Studio 在非交互式模式下也支持特定于提供商的密钥标志：
...
对于由 auth-profile 支持的提供商，这会写入 `keyRef` 条目；对于自定义提供商，这会将 `models.providers.<id>.apiKey` 写为环境变量引用（例如 `{ source: "env", provider: "default", id: "CUSTOM_API_KEY" }`）。非交互式 `ref` 模式契约：
...
### [​](https://docs.openclaw.ai/zh-CN/cli/onboard\#%E9%9D%9E%E4%BA%A4%E4%BA%92%E5%BC%8F-z-ai-%E7%AB%AF%E7%82%B9%E9%80%89%E6%8B%A9)  非交互式 Z.AI 端点选择
...
## [​](https://docs.openclaw.ai/zh-CN/cli/onboard\#%E6%B5%81%E7%A8%8B%E8%AF%B4%E6%98%8E)  流程说明
...
- 本地新手引导私信作用域行为： [CLI 设置参考](https://docs.openclaw.ai/zh-CN/start/wizard-cli-reference#outputs-and-internals)。
- 最快开始首次聊天：`openclaw dashboard`（Control UI，无需渠道设置）。
- 自定义提供商：连接任何 OpenAI 或 Anthropic 兼容端点，包括未列出的托管提供商。使用 Unknown 自动检测。
- 如果检测到 Hermes 状态，新手引导会提供迁移流程。使用 [迁移](https://docs.openclaw.ai/zh-CN/cli/migrate) 查看 dry-run 计划、覆盖模式、报告和精确映射。

## [​](https://docs.openclaw.ai/zh-CN/cli/onboard\#%E5%B8%B8%E7%94%A8%E5%90%8E%E7%BB%AD%E5%91%BD%E4%BB%A4)  常用后续命令
...
[节点](https://docs.openclaw.ai/zh-CN/cli/nodes) [配对](https://docs.openclaw.ai/zh-CN/cli/pairing)
